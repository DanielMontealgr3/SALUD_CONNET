...
try {
    $db = new database();
    $con = $db->conectar();
    $sql = "SELECT m.nom_medicamento, m.descripcion, m.codigo_barras, tm.nom_tipo_medi, i.cantidad_actual, e.nom_est, e.id_est FROM medicamentos m JOIN tipo_de_medicamento tm ON m.id_tipo_medic = tm.id_tip_medic LEFT JOIN inventario_farmacia i ON m.id_medicamento = i.id_medicamento AND i.nit_farm = :nit_farma LEFT JOIN estado e ON i.id_estado = e.id_est WHERE m.id_medicamento = :id_medicamento";
