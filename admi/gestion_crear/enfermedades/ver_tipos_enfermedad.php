...
$con = $conex->conectar();
$error_db = '';
$titulo_tabla = 'Tipos de Enfermedad';
$datos_vista = [];

$registros_por_pagina = 4; // Cambiado a 4 para consistencia
...
$columnas_tabla = ['ID', 'Nombre Tipo Enfermedad', 'Acciones'];
$total_registros = 0;
$total_paginas = 1;
...
if ($con) {
    try {
        ...
        if (!empty($filtro_nombre)) {
            $where_clauses[] = "tipo_enfermer LIKE :nombre_filtro";
        ...
        $stmt_total = $con->prepare("SELECT COUNT(*) FROM tipo_enfermedades" . $sql_where); 
        ...
        if ($total_registros > 0) {
            ...
            $sql_order_by = " ORDER BY tipo_enfermer ASC"; 
            if ($filtro_orden === 'nombre_desc') {
                $sql_order_by = " ORDER BY tipo_enfermer DESC";
            } elseif ($filtro_orden === 'recientes') { 
                $sql_order_by = " ORDER BY id_tipo_enfer DESC"; 
            }

            $sql_select = "SELECT id_tipo_enfer, tipo_enfermer FROM tipo_enfermedades" . $sql_where . $sql_order_by . " LIMIT :limit OFFSET :offset_val"; 
        ...
        error_log("PDO Ver Tipos Enfermedad: ".$e->getMessage());
        $datos_vista = [];
    }
} else {
...
<head>
    ...
    <title>Ver Tipos de Enfermedad - Administración</title>
    ...
    <main id="contenido-principal" class="flex-grow-1 d-flex flex-column">
        ...
        <div class="container-fluid mt-3 flex-grow-1 d-flex flex-column">
            <div class="vista-datos-container">
                ...
                <form method="GET" action="ver_tipos_enfermedad.php" class="mb-4 filtros-tabla-container">
                    <div class="row g-3 align-items-end">
                        <div class="col-md-4">
                            <label for="filtro_nombre" class="form-label">Buscar por Nombre:</label>
                            <input type="text" name="filtro_nombre" id="filtro_nombre" class="form-control form-control-sm" value="<?php echo htmlspecialchars($filtro_nombre); ?>" placeholder="Nombre del tipo...">
                        </div>
                        <div class="col-md-3">
                        ...
                        <div class="col-md-auto">
                            <a href="ver_tipos_enfermedad.php" class="btn btn-sm btn-limpiar-filtros-tabla w-100">Limpiar <i class="bi bi-eraser"></i></a>
                        </div>
                         <div class="col-md-auto ms-auto">
                            <a href="crear_tipo_enfermedad.php?origen=ver_tipos_enfermedad" class="btn btn-sm btn-success">Nuevo Tipo <i class="bi bi-plus-circle"></i></a>
                        </div>
                    </div>
                </form>
                            ...
                            <tbody>
                                <?php if (!empty($datos_vista)) : ?>
                                    <?php foreach ($datos_vista as $item) : ?>
                                        <tr>
                                            <td><?php echo htmlspecialchars($item['id_tipo_enfer'] ?? 'N/A'); ?></td>
                                            ...
                                            <td class="acciones-celda">
                                                <button class="btn btn-success btn-sm btn-editar-registro-enf" 
                                                        data-id="<?php echo htmlspecialchars($item['id_tipo_enfer']); ?>"
                                                        data-tipo="tipo_enfermedad"
                                                        title="Editar">
                                                    <i class="bi bi-pencil-square"></i> <span>Editar</span>
                                                </button>
                                                <button class="btn btn-danger btn-sm btn-eliminar-registro-enf" 
                                                        data-id="<?php echo htmlspecialchars($item['id_tipo_enfer']); ?>" 
                                                        data-nombre="<?php echo htmlspecialchars($item['tipo_enfermer'] ?? ''); ?>" 
                                                        data-tipo="tipo_enfermedad"
                                                        title="Eliminar">
                                ...
                                <?php else: ?>
                                     <tr><td colspan="<?php echo count($columnas_tabla); ?>" class="text-center">
                                        No hay tipos de enfermedad que coincidan con los filtros.
                                    </td></tr>
                                <?php endif; ?>
                            </tbody>
                        </table>
